...
****************************
[Script]
Bili Region = type=http-response,pattern=https:\/\/api\.bilibili\.com\/pgc\/view\/(v\d\/)?app\/season\?access_key,requires-body=1,max-size=0,control-api=1,script-path= https://raw.githubusercontent.com/arneng/Rules/master/Surge/Bilibili.js
...
if (str && obj) {
	...
	if (!notify) $notification.post(obj, ``, `${current}  =>  ${str}  =>  ${change?`⭕️`:`❌`}`);
	if (change) {
